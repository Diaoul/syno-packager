...
# CFG
my $cfg = Config::IniFiles->new(-file => "/usr/local/SABnzbd/etc/config.ini", -fallback => "GENERAL");

# Controller
if (&isAuthed()) {
	print $q->redirect("http://".$ENV{SERVER_NAME}.":".$cfg->val('misc', 'port'));
} else {
	print $q->header;
	print $q->start_html("SABnzbd - Authentication required");
